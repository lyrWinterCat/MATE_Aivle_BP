...
fp = './147.텍스트 윤리검증 데이터/01.데이터/1.Training/라벨링데이터/aihub/talksets-train-'
texts = []
types = []
is_immoral = []

print('필요한 데이터 추출...')
for i in tqdm(range(1,6), total=5, desc='NUM', ncols=100, ascii=' =', leave=True, position=0):
    fp_ = fp + f'{i}/talksets-train-{i}.json'
    with open(fp_, 'r', encoding='utf-8') as f:
...
df.to_csv("./korean_immoral_texts.csv")